	...
	if choice == "About🧾":
		...
		about_section = """
			...
			** 1. Resume Score Generator **
			<br>
			** 2. Resumme Summarizer **

			### 1. Resume Score Generator: 
			Its a Machine Learning based model which is getting executed whenever you are clicking **Process** button. It gives you a 
			score,which is generated by the model. This score might be helpful for recruters or a HR person to shortlist a candidate for interview. Except 
			that a candidate also can use it to check his/her score and according to that they can make changes to their resume.
			(The ML model which is been developed is not so much accurate,because it is trained on randomly generated score data.) 

			### 2. Resumme Summarizer:  
			Its a Custom NER(Name Entity Recognition) model which helps to summerize the resume. This is also important, because
			when the number of application increases, it gets difficult for higher ups(recruters) to look through each and every resume and analyze it.
			So, this summerizer helps to see the skills and projects of that candidate at a glance.

			**Target people** of this project is recruter or HR person of any company who will be shortlisting the candidate based on their applications.
			But, candidates also can use it to, get a score for their resume,and improve further.

			Visit the github [reposetory](https://github.com/soumya997/Resume-analyzer) to know more. If you like the project dont forget to star the 
			reposetory 😇.
